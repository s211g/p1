...
namespace rvalue_test {
    ...
    void test3() {
        ...
        auto fn3 = [](int&& r) -> int&& { return std::move(r); };

        // инициализировать r-value ссылку можно только временным значением или НЕИМЕНОВАННОЙ r-value ссылкой
        int&& rref  = int(0);
        ...
        int& ref2 = fn2(i); // ссылаться на неименованные обычные ссылки можно
